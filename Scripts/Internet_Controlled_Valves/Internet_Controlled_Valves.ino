    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          client.print("<P> <INPUT type=\"submit\" name=\"status1\"value=\"1ON\">");
          client.print("<P> <INPUT type=\"submit\" name=\"status1\"value=\"1OFF\">");
        ...
        if (buffer.indexOf("GET /?status1=1ON") >= 0)
          digitalWrite(eightChanRelayIN1, LOW);

        
        // Did the off button get pressed
        if (buffer.indexOf("GET /?status1=1OFF") >= 0)
